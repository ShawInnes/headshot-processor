...
import {NamingSettings, RenameOperation} from '../types/renaming'
import {FolderCacheService} from '../lib/folder-cache'
import {FileText, Settings} from 'lucide-react'
import {toast} from 'sonner'
import {FolderCache} from "@/types/cache.ts";
...
export function EmployeeGroupView({employeeGroup, onPhotoSelect, onFilesRenamed}: EmployeeGroupViewProps) {
    ...
    const handleStartRename = async (): Promise<RenameOperation[]> => {
        try {
            ...
            const successfulOps = results.filter((r: RenameOperation) => r.success)
                ...
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div className="text-center p-4 bg-orange-50 rounded-lg">
                            <div
                                className="text-2xl font-bold text-orange-600">{employeeGroup.unknownPhotos.length}</div>
